...
export const Header = () => {
  ...
  return (
    <>
      <header className='bg-white py-6 text-black sticky z-50 shadow-md top-0 left-0 w-full border-b-4 border-black'>
            ...
            <ul className='flex items-center gap-6 font-bold'>
              {LinkData.map((link) => (
                <li key={link.id} onClick={() => setOpen(null)}>
                  <NavLink className={({ isActive }) => (isActive ? "text-blue-800 text-sm" : "text-[15px]")} to={link.url}>
                    {link.title}
                  </NavLink>
                </li>
              ))}
